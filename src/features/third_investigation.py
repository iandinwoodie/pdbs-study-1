...
class Manager(object):
    ...
    def third_investigation(self):
        """Perform third investigation analysis."""
        ## Get relevant data.
        table = 'dogs'
        # q02_main_1 = aggression
        # q02_main_2 = fearful/anxious
        fields = 'q02_main_1, q02_main_2'
        df = self.__db.select(table, fields)
        df.columns = ['aggression', 'anxiety']
        df['aggression'] = pd.to_numeric(df['aggression'])
        df['anxiety'] = pd.to_numeric(df['anxiety'])
        ## Determine relationships.
        print('\nThird Investigation:')
        print('The Relationship Between Aggression, Fear, and Anxiety')
        print('\nSums:')
